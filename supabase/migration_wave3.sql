-- Wave 3: Charts, Dedications, Challenges

-- Weekly chart snapshots
CREATE TABLE IF NOT EXISTS chart_entries (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  week_start DATE NOT NULL, -- Monday of the chart week
  position INTEGER NOT NULL,
  prev_position INTEGER, -- NULL = new entry
  track_id UUID NOT NULL REFERENCES tracks(id) ON DELETE CASCADE,
  score NUMERIC NOT NULL DEFAULT 0,
  play_count_week INTEGER DEFAULT 0,
  unique_listeners_week INTEGER DEFAULT 0,
  ...
  is_bullet BOOLEAN DEFAULT FALSE, -- biggest mover
...
CREATE INDEX IF NOT EXISTS idx_chart_entries_track ON chart_entries(track_id, week_start DESC);
