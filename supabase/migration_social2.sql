...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE actor_name TEXT; track_owner UUID;
BEGIN
  SELECT display_name INTO actor_name FROM profiles WHERE id = NEW.user_id;
  -- Notify track owner (skip if commenter is owner)
  -- tracks table may have a different owner column — use creator_id or user_id
  -- For now store entity for manual lookup
  INSERT INTO notifications (user_id, actor_id, type, entity_id, entity_type, message)
  SELECT t.creator_id, NEW.user_id, 'comment', NEW.track_id, 'track',
    actor_name || ' commented on your track'
  FROM tracks t
  WHERE t.id = NEW.track_id AND t.creator_id != NEW.user_id AND t.creator_id IS NOT NULL;
  RETURN NEW;
END;
$$;
